	...
	public static ArrayList<Question> getQuestions(String url) {
		...
		try {
			...
			for (int i = 0; i < MAX_QUESTIONS_PER_PAGE; i++) {
				...
				String question = page.getHtmlElementById(
						QUESTION_QUERY_FIRST_HALF + (i+1)+ QUESTION_QUERY_SECOND_HALF).asText();
				for (int j = 0; j <MAX_ANSWERS_PER_PAGE; j++) {
					answers[j] = page.getHtmlElementById(ANSWER_QUERY_FIRST_HALF +
							(i+1) + "-" + (j+1) + ANSWER_QUERY_SECOND_HALF).asText();
					
					
					String correctAnswerQuery = page.getHtmlElementById(COR_ANSW_FIRST_HALF + (i+1) + "-" + (j+1) + COR_ANSW_SECOND_HALF).toString();
				...
				Question newQuestion = new Question(question,answers[0],answers[1],answers[2],
						answers[3],"NO HINTS WITH GEEKSFORGEEKS",validAnswer);
...
/*
	...
	client.getOptions().setJavaScriptEnabled(false);
		...
		try {
			...
			//System.out.println(page.getHtmlElementById("mtq_marker-9-2-1").toString());
		} catch (FailingHttpStatusCodeException | IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} */
